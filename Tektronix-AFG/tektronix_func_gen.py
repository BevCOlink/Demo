...
_VISA_ADDRESS = "USB0::0x0699::0x035E::c016439::0::INSTR"
...
class FuncGen:
    ...
    def _initialise_model_properties(self):
        ...
        if np.any(["AFG1022" in a for a in [self._model, self._override_compat]]):
            ...
            self._arbitrary_waveform_length = [2, 16000000]  # min length, max length
    ...
    def set_custom_waveform(
        ...
        """
        waveform = self._normalise_to_waveform(waveform,normalise_min,normalise_max)
        self._inst.write_binary_values(
            f"DATA:DATA EMEMory{memory_num},", waveform, datatype="H", is_big_endian=True)

    def _normalise_to_waveform(self, shape: np.ndarray,normalise_min,normalise_max) -> np.ndarray:
        ...
        waveform = shape - normalise_min
        normalisation_factor = normalise_max
    ...
    def list_to_TTL_waveform(self,sc_list, t_list=[0.2], dt=0,max=1):  # t_list[0]需是最小公倍数
        find = 'eom'
        t_total = 0
        waveform = np.array([])
        for t in t_list:
...
class FuncGenChannel:
    ...
    #     self._fgen.write(
    #         f"OUTPut{self._source}:STATe {state}", custom_err_message=err_msg
    #     )
    ...
    def set_high(self, high: float, unit: str = "V"):
        """Set offset in volts (or mV, see options)
        ...
        min_volt, max_volt = self.get_voltage_lims()
        current_amplitude = self.get_amplitude()
        high = _SI_prefix_to_factor(unit) * high
        if (
            current_amplitude / 2 - high < min_volt
            or current_amplitude / 2 + high > max_volt
        ):
            msg = (
                f"Could not set the high {high}V as the offset combined "
                f"with the amplitude ({current_amplitude}V) will be outside "
                f"the absolute voltage limits [{min_volt}, {max_volt}]V"
            )
            raise NotSetError(msg)
        # Set the offset
        cmd = f"{self._source}VOLTage:LEVel:HIGH {high}{unit}"
        err_msg = f"set high {high}{unit}"
        self._fgen.write(cmd, custom_err_message=err_msg)
        # Verify that the offset has been set
        # if self._fgen.verify_param_set:
        #     actual_offset = self.get_offset()
        #     # Multiply with the appropriate factor according to SI prefix
        #     check_offset = high * _SI_prefix_to_factor(unit)
        #     if not actual_offset == check_offset:
        #         msg = (
        #             f"Offset {high}{unit} was not set on channel "
        #             f"{self._channel}, it is {actual_offset}V. Check that the "
        #             f"number is within the possible range and in the correct "
        #             f"format.\nError from the instrument: {self._fgen.get_error()}"
        #         )
    ...
    def set_low(self, low: float, unit: str = "V"):
        """Set offset in volts (or mV, see options)
        ...
        min_volt, max_volt = self.get_voltage_lims()
        current_amplitude = self.get_amplitude()
        low = _SI_prefix_to_factor(unit) * low
        if (
            current_amplitude / 2 - low < min_volt
            or current_amplitude / 2 + low > max_volt
        ):
            msg = (
                f"Could not set the high {low}V as the offset combined "
                f"with the amplitude ({current_amplitude}V) will be outside "
                f"the absolute voltage limits [{min_volt}, {max_volt}]V"
            )
            raise NotSetError(msg)
        # Set the offset
        cmd = f"{self._source}VOLTage:LEVel:LOW {low}{unit}"
        err_msg = f"set high {low}{unit}"
        self._fgen.write(cmd, custom_err_message=err_msg)
        # Verify that the offset has been set
        # if self._fgen.verify_param_set:
        #     actual_offset = self.get_offset()
    ...
    def set_frequency(self, freq: float, unit: str = "Hz"):
        ...
        if self._fgen.verify_param_set:
            ...
            if not actual_freq == check_freq:
                ...
                raise NotSetError(msg)
    def set_trigger(self,slope:str,source:str):
        self._fgen.write(f"TRIGger:SEQuence:SLOPe {slope}",
                         custom_err_message=f"set trigger slope:{slope}")
        self._fgen.write(f"TRIGger:SEQuence:SOURce {source}",
                         custom_err_message=f"set trigger source:{source}")

    def set_burst_mode(self,idle:str):
        self._fgen.write(f"{self._source}BURSt:MODE TRIGgered",
                         custom_err_message=f"set burst mode: triggerd")
        self._fgen.write(f"{self._source}BURSt:STATe ON",
                         custom_err_message=f"enable the burst mode")
        # self._fgen.write(f"SOURce1: BURSt:NCYCles {cycs}",
        #                  custom_err_message=f"set burst cycs: {cycs}")
        self._fgen.write(f"{self._source}BURSt:IDLE {idle}",
                         custom_err_message=f"set burst idle: {idle}")

    def set_output_state(self,state:str):
        self._fgen.write(f"OUTPut{self._channel}:STATe {state}",
                         custom_err_message=f"output state: {state}")

    def set_output_mode(self,mode:str): #SYNC  TRIGger
        self._fgen.write(f"OUTPut:TRIGger:MODE {mode}",
                         custom_err_message=f"output mode: {mode}")


    def set_burst_cyc(self,cyc:int):
        self._fgen.write(f"{self._source}BURSt:NCYCles {cyc}",
                         custom_err_message=f"set burst cyc: {cyc}")

    def set_burst_cyc_inf(self):
        self._fgen.write(f"{self._source}BURSt:NCYCles INF",
                         custom_err_message=f"set burst cyc infinity")

    def set_function_shape(self,shape:str):
        self._fgen.write(f"{self._source}FUNCtion:SHAPe {shape}",
                         custom_err_message=f"set function shape: {shape}")

    def set_burst_delay(self,delay:float):
        self._fgen.write(f"{self._source}BURSt:TDELay {delay}us",
                         custom_err_message=f"{self._source}BURSt:TDELay {delay}ms")

...
class fileDialogdemo(QWidget):
    ...
    def saveFiles(self):
        save_dig=QFileDialog()
        save_dig.setFileMode(QFileDialog.AnyFile)
        save_dig.setFilter(QDir.Files)

        if save_dig.exec_():
            # 接受选中文件的路径，默认为列表
            filenames = save_dig.getSaveFileName(self,'选择保存路径','D:/ZJY-Python/20210628-计数卡扫描GUI-v1.0/test.xlsx','xlsx(*.xlsx)')
            print(filenames[0])
            # 列表中的第一个元素即是文件路径，以只读的方式打开文件
